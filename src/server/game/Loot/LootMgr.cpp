...
class LootTemplate::LootGroup                               // A set of loot definitions for items (refs are not allowed)
{
    public:
        ...
        void Process(Loot& loot, Player const* player, LootStore const& store, uint16 lootMode) const;
...
void LootTemplate::LootGroup::Process(Loot& loot, Player const* player, LootStore const& store, uint16 lootMode) const
{
    ...
    {
        sScriptMgr->OnBeforeDropAddItem(player, loot, store.IsRatesAllowed(), lootMode, const_cast<LootStoreItem*>(item), store);
        loot.AddItem(*item);
    }
}
