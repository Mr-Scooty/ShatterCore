...
void SpellMgr::LoadSpellInfoCorrections()
{
   ...
   // Yoga Flame
    ...
    // Deathwing Kill Credit (serverside): DBC targeting selects entries around
    // the source; the boss casts it on each raider explicitly at the kill
    ApplySpellFix({ 111533 }, [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
        spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo();
    });
